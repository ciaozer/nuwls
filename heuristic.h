        ...
        for (step = 1; step < max_flips; ++step)
        {
            if (hard_unsat_nb == 0)
            {
                ...
                if (soft_unsat_weight < opt_unsat_weight)
                {
                    ...
                    if (opt_time <= cutoff_time / 5){
                        opt_time_60 = opt_time;
                        opt_unsat_weight_60 = opt_unsat_weight;
                    }
                    for (int v = 1; v <= num_vars; ++v)
            ...
            if (step % 1000 == 0)
            {
                ...
                if (exceed60 == 0 && get_runtime() > cutoff_time / 5){
                    exceed60 = 1;
                    if (problem_weighted){
                        FILE * pp = fopen("NuWLS-result-W-60.xls", "a+");
                        fprintf(pp, "%s\t%lld\t%0.2f\n", inputfile, opt_unsat_weight_60, opt_time_60);
                        fclose(pp);
                    }
                    else{
                        FILE * pp = fopen("NuWLS-result-Un-60.xls", "a+");
                        fprintf(pp, "%s\t%lld\t%0.2f\n", inputfile, opt_unsat_weight_60, opt_time_60);
                        fclose(pp);
                    }
                }
